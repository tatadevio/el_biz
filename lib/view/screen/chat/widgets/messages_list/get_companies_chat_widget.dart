...
class GetCompaniesChatsWidget extends StatelessWidget {
  final String currentUserId;

  const GetCompaniesChatsWidget({super.key, required this.currentUserId});
      ...
      if (scrollController.position.pixels >=
        ...
        if (chatState.tenderSearchQuery.isNotEmpty) {
          ...
          if (chatState.currentTenderPage < chatState.pageTenderSize) {
            chatBloc.add(GetChatCompanyList(
    ...
    return RefreshIndicator(
      ...
      child: BlocBuilder<ChatBloc, ChatState>(
        builder: (context, chatState) {
          if (chatState.isLoading || chatState.isSearchingCompany) {
            return const Center(child: CircularProgressIndicator());
          }

          // Use filtered results if search is active, otherwise use original list
          final displayList = chatState.companySearchQuery.isNotEmpty
              ? chatState.filteredChatCompanyList
              : chatState.chatCompanyList;

          if (displayList.isEmpty) {
            if (chatState.companySearchQuery.isNotEmpty) {
          ...
          return ListView.builder(
            ...
            itemCount: displayList.length +
                (chatState.isLoadingCompanySearchMore ? 1 : 0),
            itemBuilder: (context, index) {
              if (index == displayList.length &&
                  chatState.isLoadingCompanySearchMore) {
                return const Center(
                  child: Padding(
