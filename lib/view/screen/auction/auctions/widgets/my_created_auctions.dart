...
class MyCreatedAuctions extends StatefulWidget {
  final String sortOrder;
  final String orderBy;
  const MyCreatedAuctions(
      {super.key, this.sortOrder = 'asc', this.orderBy = 'created_at'});

  @override
  State<MyCreatedAuctions> createState() => _MyCreatedAuctionsState();
}

class _MyCreatedAuctionsState extends State<MyCreatedAuctions> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      if (_scrollController.position.pixels >=
          ...
          !publicAuctionsBloc.state.myAuctionsLoading &&
          !publicAuctionsBloc.state.myAuctionsLoadingMore) {
        ...
        if (publicAuctionBloc.state.myAuctionsCurrentPage <
            publicAuctionBloc.state.myAuctionsTotalPages) {
          int nextPage = publicAuctionBloc.state.myAuctionsCurrentPage + 1;

          publicAuctionBloc.add(GetMyAuctions(
            page: nextPage,
            isRefresh: false,
        ...
        BlocBuilder<AuctionsBloc, AuctionsState>(
          builder: (context, auctionsController) {
            return BlocBuilder<AuctionsBloc, AuctionsState>(
              ...
              if (auctionsController.myAuctions.isEmpty) {
                          ...
                          child: Column(
                            children: [
                              GridView.builder(
                                ...
                                itemCount: auctionsController.myAuctions.length,
                                itemBuilder: (context, index) {
                                  return AuctionGridItem(
                                    auction:
                                        auctionsController.myAuctions[index],
                              ...
                              if (auctionsController.myAuctionsLoadingMore)
                                const Center(
                                  child: CircularProgressIndicator(),
                                ).paddingOnly(
                                    bottom:
