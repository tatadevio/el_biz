...
class SimilarAuctionsBloc
  ...
  Future<void> _onTogglePublicAuctionFavorite(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        bool isAuctionFavorite = false;
        for (var auction in state.similarAuctions) {
          if (auction.id.toString() == event.auctionId) {
            isAuctionFavorite = auction.isFavorite == true ? true : false;
          }
        }
        ...
        event.context.read<AuctionsBloc>().add(
              UpdateAuctionInFavoriteList(
                auctionId: int.parse(event.auctionId),
